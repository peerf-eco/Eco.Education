...
int16_t DialogProcessing() {
    ...
    Window button1, button2, button3, buttonS, button0, buttonE, buttonA, buttonD, buttonM;

    XSetWindowAttributes attrTextEdit;
    XCharStruct overall;
    XTextProperty     windowName;
    char* window_name = "Calculator 2";
    ...
    switch (g_iInfo) {
        ...
        break;
        case 3 :
            XDrawImageString(dpy, win, gc, 10, 16, "Eco.ComponentC 2.0.0.0", strlen("Eco.ComponentC 2.0.0.0"));
        break;
        case 4 :
            XDrawImageString(dpy, win, gc, 10, 16, "Eco.ComponentD 2.0.0.0", strlen("Eco.ComponentD 2.0.0.0"));
        break;
        case 5 :
            XDrawImageString(dpy, win, gc, 10, 16, "Eco.ComponentE 2.0.0.0", strlen("Eco.ComponentE 2.0.0.0"));
        break;
    }
    ...
    buttonD = XCreateWindow(dpy, win, 140, 70, 28, 28, 2, DefaultDepth(dpy, screen), InputOutput, DefaultVisual(dpy, screen), mask, &attrTextEdit);
    ...
    buttonM = XCreateWindow(dpy, win, 140, 110, 28, 28, 2, DefaultDepth(dpy, screen), InputOutput, DefaultVisual(dpy, screen), mask, &attrTextEdit);
    ...
    if (g_pIY != 0) {
        XMapWindow(dpy, buttonD);
        XMapWindow(dpy, buttonM);
    }
    ...
    while (done == 0) {
        ...
        switch (event.type) {
            case Expose:
                ...
                if (g_pIY != 0) {
                    XDrawImageString(dpy, buttonD, gc, 12, 18, "/", strlen("/"));
                    XDrawImageString(dpy, buttonM, gc, 12, 18, "*", strlen("*"));
                }
           ...
           case ButtonPress:
                ...
                if (event.xbutton.window == buttonD) {
                    XClearWindow(dpy, textEditResult);
                ...
                if (event.xbutton.window == buttonM) {
                    XClearWindow(dpy, textEditResult);
